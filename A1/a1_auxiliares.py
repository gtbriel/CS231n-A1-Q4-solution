...
def gerar_dados_de_exemplo(
  ...
  Entrada:
  - num_entradas: Inteiro N indicando o tamanho do conjunto de dados
  ...
  - exemplo_X: tensor `dtype` de shape (N, D) contendo os dados
  - exemplo_y: tensor int64 de shape (N,) contendo rótulos, onde cada elemento é um
    inteiro no intervalo [0, C)
  - params: Um dicionário dos parâmetros do modelo de exemplo, com as chaves:
    - 'W1': tensor `dtype` de shape (D, H) contendo os pesos da primeira camada
    - 'b1': tensor `dtype` de shape (H,) contendo os vieses da primeira camada
    - 'W2': tensor `dtype` de shape (H, C) contendo os pesos da segunda camada
    - 'b2': tensor `dtype` de shape (C,) contendo os vieses da segunda camada
  """
  N = num_entradas
  D = tamanho_entrada
  H = tamanho_oculta
  ...
  # Gere algumas entradas e rótulos aleatórios
...
def plotar_stats(stat_dict):
  # Plota a função de perda e as acurácias de treinamento / validação
  plt.subplot(1, 2, 1)
  plt.plot(stat_dict['historico_perda'], 'o')
  plt.title('Histórico de Perda')
  plt.xlabel('Iteração')
  plt.ylabel('Perda')

  plt.subplot(1, 2, 2)
  plt.plot(stat_dict['historico_acc_treinamento'], 'o-', label='treinamento')
  plt.plot(stat_dict['historico_acc_validacao'], 'o-', label='validacao')
...
def plotar_curvas_de_acuracia(stat_dict):
  plt.subplot(1, 2, 1)
