...
def verificacao_esparsa_grad(f, x, analitico_grad, num_verificacoes=10, h=1e-7):
  """
  ...
  - analitico_grad: Um tensor do torch contendo o gradiente analítico de f em x
  ...
  for i in range(num_verificacoes):
    ...
    grad_analitico  = analitico_grad[ix]
...
def calcular_gradiente_numerico(f, x, h=1e-7):
  ...
  achatado_x = x.contiguous().view(-1)
  grad = torch.zeros_like(x)
  achatado_grad = grad.view(-1)
